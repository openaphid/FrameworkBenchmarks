...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `brpc-java.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `brpc-java-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `brpc-java-mysql`.

4. Test your application

        $ tfb --mode verify --test brpc-java
...
Edit `.travis.yml` to ensure that Travis-CI will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=Java/brpc-java` should be inserted under `env > matrix` and put it there.
